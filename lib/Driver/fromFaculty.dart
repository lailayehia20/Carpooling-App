...
class DriverFromFaculty extends StatefulWidget {
  const DriverFromFaculty({Key? key, }) : super(key: key);

  @override
  State<DriverFromFaculty> createState() => _DriverFromFacultyState();
}

class _DriverFromFacultyState extends State<DriverFromFaculty> {
  ...
  Future<void> saveRideDetailsToFirestore() async {
    ...
    DocumentReference rideReference = await FirebaseFirestore.instance.collection('Drivers').doc(userId).collection('RidesFromFaculty').add({
      'destination': fromController.text,
      'pickup': dropdownValue2,
      'price': priceController.text,
      'destination_checkpoint': checkpointController.text,
...
bool _isTimeWithinBounds(TimeOfDay time) {
  final int upperBoundHour = 17;
  final int upperBoundMinute = 30;

  int selectedHour = time.hour;
  int selectedMinute = time.minute;

   return (selectedHour == upperBoundHour && selectedMinute == upperBoundMinute)  ;
}
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text("Heading Back From Faculty", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              ...
              child: Column(
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      const Text("Destination area:", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      const Text("Pickup:", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                      const SizedBox(width: 15),
                      Expanded(
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Enter exact destination location',
                      border: OutlineInputBorder(borderSide: BorderSide(color: Colors.black, width: 2.0)),
                    ),
                  ),
                  const SizedBox(height: 10),
